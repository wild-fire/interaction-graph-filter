...
public class InteractionUsersMapper extends InteractionDateGroupedMapper {
		...
		if(entities.get("user_mentions").getAsJsonArray().size() > 0) {
			...
			for (JsonElement mentionedUser : entities.get("user_mentions")
				...
				JsonObject mentionedUserObj = mentionedUser.getAsJsonObject(); 
					...
					this.contextWriteToGroups(context, this.tweet.isRetweeting(mentionedUserObj.get("screen_name").getAsString()) ? "retweets" : "mentions", mention);
